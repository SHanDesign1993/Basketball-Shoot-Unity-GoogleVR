...
#region Component Segments
	...
	void Start ()
	{
        t = new Teleport();
        t.ToggleVRMode();
        StartBtn.normal.background = StartBtn_T;
	...
	void OnGUI () {
        ...
        if (!bStart)
        {
            GUI.Window(1, new Rect((Screen.width - 220), 5, 210, 80), InfoWindow, "Shooting Star");
            //GUI.color = new Color(1, 1, 1, (Mathf.Sin(Time.time * 3.0f) + 1) / 2.0f);
            //GUI.DrawTexture(Rect(-Screen.width / 20, h1, Screen.width * 11 / 10, Screen.width * 77 / 100), logo);
            //GUI.color = new Color(1, 1, 1, 1);
            if (GUI.Button(new Rect(0, Screen.height * 0.5f, Screen.width, Screen.height * 0.6f), "", StartBtn))
            {
                bStart = true;
                t.ToggleVRMode();
            }
...
#region Functions
    public void SwitchSkyBefore()
    {
        SwitchSkyBox(-1);
    }

    public void SwitchSkyNext()
    {
        SwitchSkyBox(+2);
    }
	...
	{
		GUI.Label(new Rect(15, 25, 240, 20), "VR Basketball Shooting Game");
		GUI.Label(new Rect(15, 50, 240, 20), "henry8209@gmail.com");
	}
